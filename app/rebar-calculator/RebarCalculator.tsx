      ...
      <div className="p-5">
        <div className="mb-4">
          ...
          <div className="flex flex-wrap gap-2">
            {REBAR_SIZES.map((r, i) => (
              <button key={r.label} onClick={() => setRebarIdx(i)}
                className={`text-xs px-3 py-1.5 rounded-full border transition-colors ${
                  i === rebarIdx ? "bg-blue-600 border-blue-600 text-white" : "bg-white border-gray-300 text-gray-600 hover:border-blue-400"
                }`}>
                {r.label}
              </button>
            ))}
        ...
        <div className="mt-5 grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            { label: "Total Bars",   value: totalBars,              sub: "pieces" },
            { label: "Linear Feet",  value: linearFt.toFixed(0),    sub: "ft total" },
            { label: "Weight",       value: weightLbs.toFixed(0),   sub: "lbs" },
