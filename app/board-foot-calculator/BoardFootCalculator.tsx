...
function calcBF(t: number, w: number, l: number, qty: number) {
  return (t * w * l * qty) / 12;
}
...
export function BoardFootCalculator() {
  ...
  const addRow = () =>
    setBoards((prev) => [...prev, { id: Date.now(), thickness: "1", width: "6", length: "8", quantity: "1" }]);
  ...
  const totalBF = boards.reduce((sum, b) => {
    return sum + calcBF(parseFloat(b.thickness) || 0, parseFloat(b.width) || 0, parseFloat(b.length) || 0, parseInt(b.quantity) || 0);
  }, 0);
      ...
      <div className="p-5">
        <div className="space-y-3">
          {boards.map((board, idx) => {
            const bf = calcBF(parseFloat(board.thickness) || 0, parseFloat(board.width) || 0, parseFloat(board.length) || 0, parseInt(board.quantity) || 0);
            return (
              <div key={board.id} className="border border-gray-100 rounded-lg p-3 bg-gray-50">
                <div className="flex justify-between items-center mb-2">
                  ...
                  {boards.length > 1 && (
                    <button onClick={() => removeRow(board.id)} className="text-gray-300 hover:text-red-400 text-sm">Remove</button>
                  )}
                </div>
                  ...
                  {([
                    ...
                    <div key={field}>
                      ...
                      <input type="number" inputMode="decimal" min="0" step="0.25"
                        value={board[field]} onChange={(e) => updateBoard(board.id, field, e.target.value)}
        ...
        <button onClick={addRow} className="mt-3 text-xs text-blue-600 hover:text-blue-800 font-medium">
          + Add another board
        </button>
        ...
        <div className="mt-4">
          ...
          <input type="number" inputMode="decimal" min="0" step="0.25"
            value={pricePerBF} onChange={(e) => setPricePerBF(e.target.value)} className="calc-input w-32" />
        </div>
